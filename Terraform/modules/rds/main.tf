...
module "rds" {
  ...
  subnet_ids             = var.private_subnet_ids[1]      # Biến cho private subnets
...
module "rds_security_group" {
  ...
  vpc_id      = var.vpc_id  # Thêm biến vpc_id để gắn SG vào VPC
  ...
  egress_rules = [
    {
      ...
      to_port     = 0
      protocol    = "-1"
      cidr_blocks = ["0.0.0.0/0"]  # Cho phép outbound traffic
    },
    {
      from_port   = 3306
      to_port     = 3306
      protocol    = "tcp"
      cidr_blocks = [var.myIP]  # Thay bằng IP của bạn, ví dụ: "203.0.113.5/32"
    }
  ]
